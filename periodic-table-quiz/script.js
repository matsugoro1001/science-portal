...
function nextQuestion() {
    ...
    if (questionType === 0) {
        ...
        questionLabel.textContent = currentLanguage === 'JP' ? '元素記号は何？' : '元素符号是什么？';
...
function handleAnswer(selectedElement, btn) {
    ...
    if (btn.classList.contains('wrong')) return; // Prevent clicking already wrong answer

    const isCorrect = selectedElement.number === currentQuestion.element.number;

    if (isCorrect) {
        isAnswering = false; // Block input only on correct answer
        ...
        // Allow trying again (do not advance, do not reveal correct)
    }
}
...
const GAS_URL = 'https://script.google.com/macros/s/AKfycbxlO5RanxYM-eqQhBSkkQBNjmils2LLJ6UmyX7pVlAcSvly-5oePfK0xrFciVrq3Vro7g/exec';
...
async function getRankings(mode) {
    try {
        ...
        // Filter by current game mode (choice vs match)
...
const HISTORY_STORAGE_KEY = 'periodic_quiz_history';
